...
const AIModelSettings: React.FC<AIModelSettingsProps> = ({ settings, onSettingsChange }) => {
    ...
    const handleTestOpenAI = async () => {
        ...
        const success = await testOpenAIConnection(aiModelSettings.openai.apiKey);
    ...
    const handleTestOpenRouter = async () => {
        ...
        const success = await testOpenRouterConnection(aiModelSettings.openrouter.apiKey);
    ...
    const handleTestGroq = async () => {
        ...
        const success = await testGroqConnection(aiModelSettings.groq.apiKey);
    ...
    return (
        <div className="p-6 bg-black/30 backdrop-blur-lg rounded-2xl border border-cyan-400/20 shadow-2xl shadow-cyan-500/10 space-y-8">
            <div>
                 <h2 className="text-xl font-bold mb-2 text-cyan-300">تنظیمات مدل هوش مصنوعی</h2>
                 <p className="text-sm text-gray-400 mb-6">سرویس‌دهنده‌های مختلف هوش مصنوعی را مدیریت کرده و مدل پیش‌فرض برنامه را انتخاب کنید.</p>
            </div>

            <div className="p-4 bg-gray-900/30 rounded-lg border border-cyan-400/20">
                <label htmlFor="default-provider" className="block text-base font-medium text-cyan-300 mb-2">مدل پیش‌فرض برنامه</label>
                 <select
                    id="default-provider"
                    value={defaultProvider}
                    onChange={(e) => handleSettingsChange({ defaultProvider: e.target.value as any })}
                    className="w-full max-w-xs bg-gray-800/50 border border-gray-600/50 rounded-lg text-white p-2.5"
                >
                    <option value="gemini" disabled={!isProviderEnabled('gemini')}>Google Gemini (پیشنهادی)</option>
                ...
                <p className="text-xs text-gray-400 mt-2">این مدل به صورت پیش‌فرض برای تمام قابلیت‌ها استفاده می‌شود، مگر اینکه در تب "تخصیص مدل‌ها" تنظیم دیگری اعمال شود.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-4 p-4 border border-gray-700 rounded-lg bg-gray-900/30">
                    ...
                    <div>
                        ...
                        <input id="openai-apiKey" name="apiKey" type="password" value={aiModelSettings.openai.apiKey} onChange={(e) => handleApiKeyChange('openai', 'apiKey', e.target.value, setOpenaiStatus)} placeholder="کلید API خود را وارد کنید (sk-...)" className="w-full bg-gray-800/50 border border-gray-600/50 rounded-lg text-white placeholder-gray-400 focus:ring-cyan-500 focus:border-cyan-500 p-2.5"/>
                    </div>
                     <div className="flex items-center gap-2">
                        <button onClick={handleTestOpenAI} disabled={!aiModelSettings.openai.apiKey || openaiStatus === 'testing'} className="text-sm bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 px-3 rounded-lg transition disabled:opacity-50">تست اتصال</button>
                ...
                <div className="space-y-4 p-4 border border-gray-700 rounded-lg bg-gray-900/30 md:col-span-2">
                    ...
                    <p className="text-xs text-gray-400">برای استفاده از مدل‌های خاص یا رایگان، نام کامل مدل را از سایت OpenRouter کپی کرده و در فیلد بالا وارد کنید.</p>
                     <div className="flex items-center gap-2">
                        <button onClick={handleTestOpenRouter} disabled={!aiModelSettings.openrouter.apiKey || openrouterStatus === 'testing'} className="text-sm bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 px-3 rounded-lg transition disabled:opacity-50">تست اتصال</button>
                ...
                <div className="space-y-4 p-4 border border-gray-700 rounded-lg bg-gray-900/30">
                    ...
                    <div>
                        ...
                        <input id="groq-apiKey" name="apiKey" type="password" value={aiModelSettings.groq.apiKey} onChange={(e) => handleApiKeyChange('groq', 'apiKey', e.target.value, setGroqStatus)} placeholder="کلید API خود را وارد کنید (gsk_...)" className="w-full bg-gray-800/50 border border-gray-600/50 rounded-lg text-white placeholder-gray-400 focus:ring-cyan-500 focus:border-cyan-500 p-2.5"/>
                    </div>
                     <div className="flex items-center gap-2">
                        <button onClick={handleTestGroq} disabled={!aiModelSettings.groq.apiKey || groqStatus === 'testing'} className="text-sm bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 px-3 rounded-lg transition disabled:opacity-50">تست اتصال</button>
